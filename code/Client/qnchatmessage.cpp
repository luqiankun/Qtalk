...
QNChatMessage::QNChatMessage(QWidget* parent) : QWidget(parent) {
    ...
    //    te_font.setLetterSpacing(QFont::PercentageSpacing, 100);
    //    //300%,100为默认  //设置字间距%
    //    te_font.setLetterSpacing(QFont::AbsoluteSpacing, 0); //设置字间距为3像素
    //    //设置字间距像素值
    this->setFont(te_font);
    m_leftPixmap = QPixmap(":/img/zuo.png");
    m_rightPixmap = QPixmap(":/img/you.png");
...
QSize QNChatMessage::fontRect(QString str) {
    ...
    int sanJiaoW = 6;
    int kuangTMP = 30;
    int textSpaceRect = 22;
    m_kuangWidth = this->width() - kuangTMP - 2 * (iconWH + iconSpaceW + iconRectW);
    m_textWidth = m_kuangWidth - 2 * textSpaceRect - 20;
...
void QNChatMessage::paintEvent(QPaintEvent* event) {
    ...
    if (m_userType == User_Type::User_She) {  // 用户
        ...
        QTextOption option(Qt::AlignLeft | Qt::AlignVCenter);
        option.setWrapMode(QTextOption::WrapAtWordBoundaryOrAnywhere);
        painter.setFont(QFont(this->font()));
