...
bool SQL::Login(QString useName, QString userPwd) {
    if (!db->open()) {
        ...
        QSqlQuery query;
        QString str = QString::fromLocal8Bit("select * from user_pwd where 昵称=? and 密码=?;");
        query.prepare(str);
        ...
        if (!query.first()) {
            ...
            QString str2 = QString::fromLocal8Bit("SELECT * FROM online_user WHERE 昵称=") + useName + ";";
            query.exec(str2);
            rec = query.record();
            if (!query.first()) {
                // TODO
                str2.clear();
                str2 = QString::fromLocal8Bit("insert into online_user (昵称) values(") + useName + ");";
                query.exec(str2);
                qDebug() << useName + QString::fromLocal8Bit("登录成功");
                return true;
            }
            else {
...
bool SQL::Logout(QString useName) {
    if (!db->open()) {
        ...
        QSqlQuery query;
        QString str = QString::fromLocal8Bit("select * from user_pwd where 昵称=\"") + useName + "\"";
        query.exec(str);
        QSqlRecord rec = query.record();
        if (!query.first()) {
            PrintStr("不存在该账号");
            ...
            if (query.first()) {
                // TODO
                str2.clear();
                str2 = QString::fromLocal8Bit("delete from online_user where 昵称=\"") + useName + "\"";
                query.exec(str2);
...
bool SQL::IsOnline(QString useName) {
    if (!db->open()) {
        ...
        QSqlQuery query;
        QString str = QString::fromLocal8Bit("select * from online_user where 昵称=") + useName + ";";
        query.exec(str);
        if (!query.first()) {
...
bool SQL::Logon(QString useName, QString userPwd) {
    if (!db->open()) {
        ...
        QSqlQuery query;
        QString str = QString::fromLocal8Bit("select * from user_pwd where 昵称=?;");
        query.prepare(str);
        query.addBindValue(useName);
        query.exec();
        if (query.first()) {
...
bool SQL::Upheadimg(QString useName, QString userPwd, QString imgdata) {
    if (!db->open()) {
        ...
        QSqlQuery query;
        QString str = QString::fromLocal8Bit("update user_pwd set 头像=\"") + imgdata + QString::fromLocal8Bit("\" where 昵称=? and 密码=?;");
...
QString SQL::HeadOf(QString useName, QString) {
    if (!db->open()) {
        ...
        QString str = QString::fromLocal8Bit("select 头像 from user_pwd where 昵称=?;");
        QSqlQuery query;
        query.prepare(str);
        query.addBindValue(useName);
        if (query.exec()) {
            if (query.first()) {
                return query.value(QString::fromLocal8Bit("头像")).toString();
            }
            else {
                return "";
            }
...
QList<QString> SQL::OnlineList() {
    ...
    if (!db->open()) {
        ...
        while (query.next()) {
            rec = query.record();
            int snamecol = rec.indexOf(QString::fromLocal8Bit("昵称"));
